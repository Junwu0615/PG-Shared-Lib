    ...
    def send_message(self, topic, key, payload):
        ...
        # 緩衝區滿了 queue.buffering.max.messages 的限制，無法接受更多訊息
            ...
            self.logging.warning(
                f"Local producer queue is full"
                f" ({len(self.producer)} messages awaiting delivery), waiting ..."
            )
            ...
            self.logging.error(
                f"Failed to produce message to {topic} [KEY: {key}]", exc_info=True
            )
