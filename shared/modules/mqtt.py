...
class MqttServer:
    ...
    def clear_retained_message(self, topic: str, fun: callable = None, **kwargs):
        ...
        try:
            ...
            self.logging.info(
                f"成功向 Topic '{topic}' 發布空的保留訊息，以清除舊資料", stack_level=2
            )

        except Exception as e:
            self.logging.error(
                f"清除 Topic '{topic}' 的保留訊息時發生錯誤", stack_level=2
            )

    def subscriber_topic(
        self,
        subscriber_list: list = None,
        ...
        **kwargs,
        ...
        mc.on_connect = (
            self.on_connect_subscriber
            if on_connect_subscriber is None
            else on_connect_subscriber
        )
    ...
    def middle_server_thread(self, **kwargs):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            try:
                ...
                self.logging.notice(
                    "TCP 服務準備停止，正在關閉 Socket ...", stack_level=0
                )
        ...
        try:
            ...
            while not self.stop_event.is_set():
                ...
                if len(buffer) > MAX_MSG_SIZE:
                    self.logging.error(
                        "接收到的數據超過最大限制，關閉連線 ...", stack_level=2
                    )
                    conn.close()
                    break
            ...
            if buffer:
                try:
                    ...
                    if topic and payload is not None and qos is not None:
                        ...
                        self.logging.error(
                            f"無效的 JSON 格式或缺少 topic/payload/qos 鍵",
                            exc_info=False,
                            stack_level=2,
                        )
                    ...
                    self.logging.error(
                        f"接收到無效的 JSON 數據：{buffer}", stack_level=2
                    )
    ...
    def publisher_server(
        self,
        on_connect_publisher: callable = None,
        ...
        **kwargs,
        ...
        self.logging.info(
            f"[{thread_name}] 啟動 MQTT 發布執行緒池 [{self.max_workers}] ...",
            stack_level=2,
        )
        try:
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                futures = [
                    executor.submit(
                        self._publisher_worker, queue_type, f"{thread_name}_worker"
                    )
                ...
                for future in as_completed(futures):
                    try:
                        ...
                        self.logging.error(
                            f"[{thread_name}] 發布者執行緒發生錯誤", stack_level=2
                        )
    ...
    def _publisher_worker(
        self, todo_queue: Queue = None, worker_title: str = "publisher_worker"
    ):
        # ------------------------------------
        # 3.3.4) MQTT 執行緒函式
        ...
        mc = mqtt_client.Client(
            client_id=f"{self.client_id}_PublisherWorker_{threading.get_ident()}"
        )
    ...
    def send_to_middle(
        ...
        **kwargs,
        ...
        try:
            ...
            self.logging.error(
                f"連接失敗...請確認 MQTT 服務程式已啟動，"
                f"並運行在 {self.middle_host}:{self.middle_port}",
                exc_info=False,
                stack_level=2,
            )
    ...
    def on_connect_publisher(self, client, userdata, flags, rc, **kwargs):
        ...
        if rc == 0:
            ...
            self.logging.error(
                f"連接失敗...錯誤碼: {rc}", exc_info=False, stack_level=2
            )
    ...
    def on_message(self, client, userdata, msg, **kwargs):
        ...
        try:
            ...
            self.logging.notice(
                f"client: {client}, topic: {topic}, payload: {payload}", stack_level=0
            )
            # pass
